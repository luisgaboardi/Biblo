    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid gap-3">
        ...
        {lessons?.map((lesson) => (
          <div
            key={lesson.id}
            className="bg-white p-4 rounded-2xl border-2 border-b-4 border-gray-200 flex justify-between items-center hover:border-gray-300 transition-all"
          >
            <div>
              <h3 className="font-black text-gray-800">{lesson.title}</h3>
              <div className="flex gap-2 mt-1">
                <span className="text-[10px] font-black bg-blue-100 text-biblo-blue px-2 py-0.5 rounded-full uppercase">
                  {lesson.book}
                </span>
            ...
            <div className="flex gap-2">
              <button
                ...
                aria-label={`Editar lição ${lesson.title}`}
                className="p-2 text-biblo-blue hover:bg-blue-50 rounded-lg transition-colors cursor-pointer"
              >
                <Pencil size={18} />
              </button>
              <button
