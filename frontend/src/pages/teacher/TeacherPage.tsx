...
export function TeacherPage({ onLogout }: TeacherPageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f7] pb-20">
      ...
      <Header title="BIBLO PROFESSOR" onLogout={onLogout}>
        <button
          type="button"
          onClick={() => setView('list')}
          className={`px-4 py-2 rounded-xl font-bold text-sm cursor-pointer transition-all ${view === 'list' ? 'bg-biblo-blue text-white shadow-[0_3px_0_0_#1a73e8]' : 'text-gray-400'}`}
        >
          LIÇÕES
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-xl font-bold text-sm cursor-pointer transition-all ${view === 'editor' && !currentLesson.id ? 'bg-biblo-blue text-white shadow-[0_3px_0_0_#1a73e8]' : 'text-gray-400'}`}
        >
          + NOVA LIÇÃO
        </button>
      </Header>
