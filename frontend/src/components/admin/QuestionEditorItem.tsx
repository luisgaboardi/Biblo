...
export function QuestionEditorItem({ q, index, updateQuestion, removeQuestion }: Props) {
    return (
        <div className="bg-white p-6 rounded-3xl border-2 border-b-4 border-gray-200 space-y-4 relative animate-fadeIn">
            <div className="flex justify-between items-center">
                <span className="bg-biblo-blue text-white px-3 py-1 rounded-full font-black text-[12px]">QUESTÃO {index + 1}</span>
                <button onClick={() => removeQuestion(index)} className="text-red-500 cursor-pointer font-bold text-xs uppercase hover:text-red-700">Remover</button>
            </div>
            ...
            {q.type === 'order_sequence' && (
                <OrderSequenceEditor question={q} index={index} updateQuestion={updateQuestion} />
            )}
